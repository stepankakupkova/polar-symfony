...
final class RectangleWriteController
{
    private string $imageDefault = 'data/banner/rectangle/!default-banner.png';
    ...
    public function add(Request $request): Response
    {
            ...
            if (empty($errors)) {
                try {
                    ...
                    $data = [
                        ...
                        'rank'         => $this->rectangleRepository->getCount() + 1,
                    ...
                    $id = $this->rectangleRepository->insertPost($data);

                    // Adresář
                    $folder = 'data/banner/rectangle/' . $id;
                    ...
                    $this->rectangleRepository->updatePost($id, ['image' => $image]);
                    ...
                    // Log
                    $this->logger->notice('RECTANGLE - Add banner', [
                        'description' => 'OK',
                    ...
                    // Log
                    $this->logger->err('RECTANGLE - Add banner', [
                        'description' => 'ERROR',
                        'user'        => $identity->getUserIdentifier(),
                        'file'        => __FILE__,
                        'trace'       => $e->getMessage(),
        ...
        return new Response($this->renderer->renderWithAdminLayout('banner/admin/rectangle/add', [
            'pageTitle'    => 'Rectangle',
            'post'         => $post,
            'errors'       => $errors,
    ...
    public function edit(Request $request, int $id): Response
    {
        ...
        if ($request->isMethod('POST')) {
            ...
            if (empty($errors)) {
                try {
                    // Obrázek
                    $image = $post['image'] ?? $rectangle['image'];
                    if ($image === $this->imageDefault) {
                        $image = null;
                    }
                    ...
                    $this->rectangleRepository->updatePost($id, $data);
                    ...
                    // Log
                    $this->logger->notice('RECTANGLE - Edit banner', [
                        'description' => 'OK',
                        'user'        => $identity->getUserIdentifier(),
                        'file'        => __FILE__,
                    ]);
                    ...
                    // Log
                    $this->logger->err('RECTANGLE - Edit banner', [
                        'description' => 'ERROR',
                        'user'        => $identity->getUserIdentifier(),
                        'file'        => __FILE__,
                        'trace'       => $e->getMessage(),
    ...
    public function deleteBanner(Request $request): JsonResponse
    {
        ...
        try {
            $rectangle_id = $request->request->getInt('id');

            $rectangle = $this->rectangleRepository->findPostBy('id', $rectangle_id);

            // Smazat adresář
            $dir = $this->PUBLIC_PATH . '/data/banner/rectangle/' . $rectangle['id'] . '/';
            if (is_dir($dir)) {
                $this->deleteDir($dir);
            }

            $this->rectangleRepository->deletePost($rectangle_id);

            // Log
            $this->logger->notice('RECTANGLE - Delete banner', [
                'description' => 'OK',
                'user'        => $identity->getUserIdentifier(),
                'file'        => __FILE__,
            ]);
            ...
            // Log
            $this->logger->err('RECTANGLE - Delete banner', [
                'description' => 'ERROR',
                'user'        => $identity->getUserIdentifier(),
                'file'        => __FILE__,
                'trace'       => $message,
    ...
    public function uploadImage(Request $request): JsonResponse
    {
        ...
        $rectangle_id = $request->request->get('rectangle_id');

        $folder = 'data/banner/rectangle/';
        if ($rectangle_id !== 'null' && $rectangle_id !== null) {
            $folder .= $rectangle_id . '/';
        } else {
            $folder .= 'tmp/';
        }
        ...
        if ($rectangle_id !== 'null' && $rectangle_id !== null) {
            ...
            if (!empty($rectangle['image']) && $rectangle['image'] !== $this->imageDefault) {
                @unlink($this->PUBLIC_PATH . '/' . $rectangle['image']);
            }

            $this->rectangleRepository->updatePost((int) $rectangle_id, ['image' => $imageFileName]);
        }
    ...
    public function setDefaultImage(Request $request): JsonResponse
    {
        ...
        try {
            $rectangle_id = $request->request->getInt('rectangle_id');
            ...
            $this->rectangleRepository->updatePost($rectangle_id, [
                'image'        => null,
                'updated_date' => date('Y-m-d H:i:s'),
                'updated_user' => $identity->getUserIdentifier(),
            ]);
            ...
            // Log
            $this->logger->notice('RECTANGLE - Set banner image', [
                'description' => 'OK',
                'user'        => $identity->getUserIdentifier(),
                'file'        => __FILE__,
            ]);
            ...
            // Log
            $this->logger->err('RECTANGLE - Set banner image', [
                'description' => 'ERROR',
                'user'        => $identity->getUserIdentifier(),
                'file'        => __FILE__,
                'trace'       => $message,
    ...
    private function saveBannerSort(array $data, int $rank = 1): int
    {
        if ($data) {
            ...
            foreach ($data as $item) {
                $this->rectangleRepository->updatePost((int) $item['id'], [
                    'rank'         => $rank,
                    'updated_date' => date('Y-m-d H:i:s'),
                    'updated_user' => $identity->getUserIdentifier(),
                ]);
