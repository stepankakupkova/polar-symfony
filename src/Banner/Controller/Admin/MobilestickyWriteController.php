...
final class MobilestickyWriteController
{
    private string $imageDefault = 'data/banner/mobilesticky/!default-banner.png';
    ...
    public function add(Request $request): Response
    {
            ...
            if (empty($errors)) {
                try {
                    ...
                    $data = [
                        ...
                        'rank'         => $this->mobilestickyRepository->getCount() + 1,
                    ...
                    $id = $this->mobilestickyRepository->insertPost($data);

                    // Adresář
                    $folder = 'data/banner/mobilesticky/' . $id;
                    ...
                    $this->mobilestickyRepository->updatePost($id, ['image' => $image]);
                    ...
                    // Log
                    $this->logger->notice('MOBILESTICKY - Add banner', [
                        'description' => 'OK',
                    ...
                    // Log
                    $this->logger->err('MOBILESTICKY - Add banner', [
                        'description' => 'ERROR',
                        'user'        => $identity->getUserIdentifier(),
                        'file'        => __FILE__,
                        'trace'       => $e->getMessage(),
        ...
        return new Response($this->renderer->renderWithAdminLayout('banner/admin/mobilesticky/add', [
    ...
    public function edit(Request $request, int $id): Response
    {
        ...
        if ($request->isMethod('POST')) {
            ...
            if (empty($errors)) {
                try {
                    // Obrázek
                    $image = $post['image'] ?? $mobilesticky['image'];
                    if ($image === $this->imageDefault) {
                        $image = null;
                    }
                    ...
                    $this->mobilestickyRepository->updatePost($id, $data);
                    ...
                    // Log
                    $this->logger->notice('MOBILESTICKY - Edit banner', [
                        'description' => 'OK',
                        'user'        => $identity->getUserIdentifier(),
                        'file'        => __FILE__,
                    ]);
                    ...
                    // Log
                    $this->logger->err('MOBILESTICKY - Edit banner', [
                        'description' => 'ERROR',
                        'user'        => $identity->getUserIdentifier(),
                        'file'        => __FILE__,
                        'trace'       => $e->getMessage(),
    ...
    public function deleteBanner(Request $request): JsonResponse
    {
        $success        = true;
        $message        = null;
        $mobilesticky_id = null;

        $identity = $this->security->getUser();

        try {
            $mobilesticky_id = $request->request->getInt('id');

            $mobilesticky = $this->mobilestickyRepository->findPostBy('id', $mobilesticky_id);

            // Smazat adresář
            $dir = $this->PUBLIC_PATH . '/data/banner/mobilesticky/' . $mobilesticky['id'] . '/';
            if (is_dir($dir)) {
                $this->deleteDir($dir);
            }

            $this->mobilestickyRepository->deletePost($mobilesticky_id);

            // Log
            $this->logger->notice('MOBILESTICKY - Delete banner', [
                'description' => 'OK',
                'user'        => $identity->getUserIdentifier(),
                'file'        => __FILE__,
            ]);
            ...
            // Log
            $this->logger->err('MOBILESTICKY - Delete banner', [
                'description' => 'ERROR',
                'user'        => $identity->getUserIdentifier(),
                'file'        => __FILE__,
                'trace'       => $message,
        ...
        return new JsonResponse([
            'success'        => $success,
            'message'        => $message,
            'mobilesticky_id' => $mobilesticky_id,
        ]);
    }
    ...
    public function uploadImage(Request $request): JsonResponse
    {
        ...
        $mobilesticky_id = $request->request->get('mobilesticky_id');

        $folder = 'data/banner/mobilesticky/';
        if ($mobilesticky_id !== 'null' && $mobilesticky_id !== null) {
            $folder .= $mobilesticky_id . '/';
        } else {
            $folder .= 'tmp/';
        }
        ...
        if ($mobilesticky_id !== 'null' && $mobilesticky_id !== null) {
            ...
            if (!empty($mobilesticky['image']) && $mobilesticky['image'] !== $this->imageDefault) {
                @unlink($this->PUBLIC_PATH . '/' . $mobilesticky['image']);
            }

            $this->mobilestickyRepository->updatePost((int) $mobilesticky_id, ['image' => $imageFileName]);
        }
    ...
    public function setDefaultImage(Request $request): JsonResponse
    {
        ...
        try {
            $mobilesticky_id = $request->request->getInt('mobilesticky_id');
            ...
            $this->mobilestickyRepository->updatePost($mobilesticky_id, [
                'image'        => null,
                'updated_date' => date('Y-m-d H:i:s'),
                'updated_user' => $identity->getUserIdentifier(),
            ]);
            ...
            // Log
            $this->logger->notice('MOBILESTICKY - Set banner image', [
                'description' => 'OK',
                'user'        => $identity->getUserIdentifier(),
                'file'        => __FILE__,
            ]);
            ...
            // Log
            $this->logger->err('MOBILESTICKY - Set banner image', [
                'description' => 'ERROR',
                'user'        => $identity->getUserIdentifier(),
                'file'        => __FILE__,
                'trace'       => $message,
    ...
    private function saveBannerSort(array $data, int $rank = 1): int
    {
        if ($data) {
            ...
            foreach ($data as $item) {
                $this->mobilestickyRepository->updatePost((int) $item['id'], [
                    'rank'         => $rank,
                    'updated_date' => date('Y-m-d H:i:s'),
                    'updated_user' => $identity->getUserIdentifier(),
                ]);
