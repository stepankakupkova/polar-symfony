...
final class ShowexWriteController
{
	private string $imageDefault = 'data/mimoradne/show/!default-show.png';
		...
		if ($request->isMethod('POST')) {
			...
			try {
				$data = [
					...
					'order' => $showexRepository->getCount() + 1,
				];
				...
				$showexRepository->updatePost($show_id, [
					'image' => $image,
					'thumb' => $image,
				]);

				// Redactor
				$dirFile = $this->PUBLIC_PATH . '/data/mimoradne/show/tmp/file';
				if (is_dir($dirFile)) {
				...
				$dirImage = $this->PUBLIC_PATH . '/data/mimoradne/show/tmp/image';
				if (is_dir($dirImage)) {
	...
	): Response
	{
		...
		if ($show_id === 0) {
			return new RedirectResponse($urlGenerator->generate('admin_program_showex_add'));
		}
	...
	): JsonResponse
	{
		...
		try {
			...
			if ($show) {
				$this->deleteDir($this->PUBLIC_PATH . '/data/mimoradne/show/' . $show['id'] . '/');

				$programRepository->deleteProgram2Shows($show['id']);
				$showexRepository->deletePost($show['id']);

				// Re-order
				$shows = $showexRepository->fetchAll();
				$rank = 1;
				foreach ($shows as $s) {
					$showexRepository->updatePost($s['id'], ['order' => $rank]);
					$rank++;
				}
	...
	): JsonResponse
	{
		...
		try {
			...
			foreach ($data as $item) {
				$showexRepository->updatePost($item['id'], ['order' => $rank]);
				$rank++;
			}
		} catch (Exception $e) {
			$success = false;
	...
	): JsonResponse
	{
		...
		if ($show_id && $show_id !== 'null') {
			...
			$showexRepository->updatePost((int) $show_id, [
				'image' => $imageFileName,
				'thumb' => $imageFileName,
			]);
		}
