...
final class VideoexWriteController
{
	...
	): Response
	{
		...
		if ($request->isMethod('POST')) {
			...
			try {
				...
				// Log
				$logger->err('PROGRAM - Edit extraordinary video', [
					'description' => 'ERROR',
					'user' => $identity->getUserIdentifier(),
					'file' => __FILE__,
					'trace' => $e->getMessage(),
	...
	): JsonResponse
	{
		...
		try {
			...
			if ($video) {
				...
				// Log
				$logger->err('PROGRAM - Delete extraordinary video', [
					'description' => 'ERROR',
					'user' => $identity->getUserIdentifier(),
					'file' => __FILE__,
					'trace' => $message,
	...
	): Response
	{
		...
		return new StreamedResponse(function() use ($videoexRepository, $settingRepository, $logger, $identity, $cron, &$message) {
			...
			try {
				...
				$logger->err('PROGRAM - Load extraordinary videos', [
					'description' => 'ERROR',
					'user' => $identity ? $identity->getUserIdentifier() : 'CRON',
					'file' => __FILE__,
					'trace' => $e->getMessage(),
	...
	): JsonResponse
	{
		...
		try {
			...
			// Log
			$logger->err('PROGRAM - Add video part', [
				'description' => 'ERROR',
				'user' => $identity->getUserIdentifier(),
				'file' => __FILE__,
				'trace' => $message,
	...
	): JsonResponse
	{
		...
		try {
			...
			// Log
			$logger->err('PROGRAM - Delete extraordinary video part', [
				'description' => 'ERROR',
				'user' => $identity->getUserIdentifier(),
				'file' => __FILE__,
				'trace' => $message,
	...
	): JsonResponse
	{
		...
		try {
			...
			if ($video) {
				...
				// Log
				$logger->err('PROGRAM - Edit extraordinary video - Create preview', [
					'description' => 'ERROR',
					'user' => $identity->getUserIdentifier(),
					'file' => __FILE__,
					'trace' => $message,
	...
	private function createPreview(string $file, int $sec, Logger $logger, mixed $identity): void
	{
		...
		try {
			...
			$logger->err('PROGRAM - Load extraordinary videos - Create preview', [
				'description' => 'ERROR',
				'user' => $identity ? $identity->getUserIdentifier() : 'CRON',
				'file' => __FILE__,
				'trace' => $e->getMessage(),
